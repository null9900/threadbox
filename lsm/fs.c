...
static ssize_t funcsandbox_listen_socket(struct file *file, const char __user *buf, size_t count, loff_t *ppos){
  ...
  pr_info("ok = %d\n", res);
  restrict_listen_port(pid, thread_id, res);
  return count;
}
...
static ssize_t funcsandbox_ioctl(struct file *file, const char __user *buf, size_t count, loff_t *ppos){
  pid_t thread_id = current->pid;
  pid_t pid = current->tgid;
  int res;
  if(kstrtoint_from_user(buf, count, 10, &res)){
  ...
  pr_info("ok ioctl = %d\n", res);
  restrict_ioctl(pid, thread_id, res);
  return count;
}

static ssize_t funcsandbox_fcntl(struct file *file, const char __user *buf, size_t count, loff_t *ppos){
  pid_t thread_id = current->pid;
  pid_t pid = current->tgid;
  int res;
  if(kstrtoint_from_user(buf, count, 10, &res)){
  ...
  pr_info("ok fcntl = %d\n", res);
  restrict_fcntl(pid, thread_id, res);
  return count;
}
...
static ssize_t funcsandbox_disable_all(struct file *file, const char __user *buf, size_t count, loff_t *ppos){
  pid_t pid = current->tgid;
  pid_t thread_id = current->pid;
  restrict_all(pid, thread_id);
  return count;
}
...
const struct file_operations listen_socket_ops = {
  .write	= funcsandbox_listen_socket,
...
const struct file_operations ioctl_ops = {
  .write	= funcsandbox_ioctl,
  .llseek = generic_file_llseek
};

const struct file_operations fcntl_ops = {
  .write	= funcsandbox_fcntl,
  .llseek = generic_file_llseek
};
...
const struct file_operations disable_all_ops = {
  .write	= funcsandbox_disable_all,
  .llseek = generic_file_llseek
};
...
static __init int create_fs_nodes(void){
  struct dentry *funcsandbox_dir = securityfs_create_dir(FS_FOLDER_NAME, NULL);
	securityfs_create_file("listen_socket", 0666, funcsandbox_dir, NULL, &listen_socket_ops);
	securityfs_create_file("bind_socket", 0666, funcsandbox_dir, NULL, &bind_socket_ops);
	securityfs_create_file("ioctl", 0666, funcsandbox_dir, NULL, &ioctl_ops);
	securityfs_create_file("fcntl", 0666, funcsandbox_dir, NULL, &fcntl_ops);
	securityfs_create_file("fork", 0666, funcsandbox_dir, NULL, &fork_ops);
	securityfs_create_file("disable_all", 0666, funcsandbox_dir, NULL, &disable_all_ops);
